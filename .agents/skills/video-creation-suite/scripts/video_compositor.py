...
def _resize(clip, resolution):
    if hasattr(clip, 'resized'):
        return clip.resized(resolution)
    return clip.resize(resolution)

def _volumex(clip, volume):
    return clip.with_volume_scaled(volume) if hasattr(clip, 'with_volume_scaled') else clip.volumex(volume)
...
def composite_video(
    ...
    try:
        ...
        video_clip = _set_duration(video_clip, len(image_files) * duration_per_image)
        video_clip = _resize(video_clip, resolution)
        ...
        if audio_dir and os.path.exists(audio_dir):
            ...
            for bgm_file in bgm_files:
                ...
                bgm_clip = _set_duration(bgm_clip, video_clip.duration)
                bgm_clip = _volumex(bgm_clip, 0.3)  # 背景音乐音量 30%
            ...
            for idx, voice_file in enumerate(voice_files):
                ...
                voice_clip = _volumex(voice_clip, 1.0)  # 配音音量 100%
                start_time = idx * duration_per_image
                voice_clip = _set_start(voice_clip, start_time)
                audio_clips.append(voice_clip)
                logger.log_info(f"添加配音: {voice_file}，於 {start_time} 秒播放")
            ...
            for sfx_file in sfx_files:
                ...
                sfx_clip = _volumex(sfx_clip, 0.5)  # 音效音量 50%
        ...
        if audio_clips:
            ...
            final_audio = _set_duration(final_audio, video_clip.duration)
