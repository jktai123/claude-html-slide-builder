...
description: |
  ...
  5. 互動式心智圖（Mindmap，利用 HTML/CSS/SVG 繪製結構與關聯，一律包含 1 頁）
...
---## 1. 分析大綱與美學設定（調色盤、視覺 Motif、Sandwich 結構與心智圖）

分析教材後，**直接主動決定**哪些頁面要套用五大強化功能（且**一律包含至少 1 頁心智圖**），並**主動為簡報挑選視覺風格**。**請勿詢問使用者要在哪裡加什麼或使用什麼主題，一切由你根據教材內容主動決定！**
...
4. **規劃心智圖頁面**：**一律規劃一頁心智圖**，放置於簡報後半部或結尾前，用於總結教材核心關係。
...
- [VIZ:mindmap] 互動心智圖（一律包含 1 頁，用 SVG/CSS 繪製結構）
...
| [VIZ:mindmap] | 全局概念整合、結構複習（結尾或過渡前） | **必選 1 頁** |
...
## 7. 部署與發佈至 github.io Portal 門戶

不新建獨立的 GitHub 倉庫，而是統一發佈至 `/Users/jktai/web/svn12.github.io/Gemini/` 底下，並自動登錄至總覽 Portal `生成Gemini.html` 中。

### 1. 執行自動發佈腳本
呼叫發佈腳本，搬移檔案並自動插入卡片至首頁：
```bash
python3 /Users/jktai/.gemini/config/skills/publish-to-github-io/scripts/publish.py \
  --src "<來源資料夾絕對路徑>" \
  --dest-dir "<子目錄英文縮寫>" \
  --title "<簡報標題>" \
  --desc "<簡報簡介描述>" \
...
### 2. 同步並推播至 GitHub
前往 `svn12.github.io` 本地倉庫進行暫存、拉取最新變更與推送：
```bash
cd /Users/jktai/web/svn12.github.io
git stash
git pull --rebase
git stash pop
git push
...
```
✅ 簡報已發佈並更新 Portal！
🔗 簡報網址：https://svn12.github.io/Gemini/<子目錄名稱>/index.html
🔗 總覽門戶：https://svn12.github.io/Gemini/生成Gemini.html
```

---

## 8. 手機版 (Mobile Portrait) RWD 最佳化

為了確保簡報在手機垂直螢幕上能獲得完美的閱讀體驗：
1. **Reveal.js 動態視窗初始化**：
   在初始化 Reveal.js 時，自動偵測視窗比例與寬度。若偵測為垂直手機畫面，將 `width` 與 `height` 設為 `450x800` (即 9:16)，並將 `margin` 縮小（如 `0.05`），這樣可以使 Reveal.js 的自動縮放機制完美適配手機垂直排版，免除字體變 microscopic 的問題。
2. **多欄排版塌縮 (Collapse)**：
   在 CSS 模版中配置 RWD 媒體查詢（Media Query）。當螢幕寬度小於 `560px` 或為垂直螢幕時：
   - 所有的 `.adv-grid`、`.two-col-grid`、`.three-card-grid` 等多欄佈局必須強制轉為單欄排版 (`grid-template-columns: 1fr !important;`)。
   - 縮小標題與內容字體，避免在窄螢幕上產生溢出或過度換行。
   - 調整 `VIZ` 滑桿、文字雲等元件為 100% 寬度自適應。
