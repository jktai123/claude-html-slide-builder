...
### FlowCards（流程卡）
- 用途：展示步骤、流程或演进阶段，以横向排列的卡片形式呈现，避免单一 Bullet List。
- 结构：标题 + 矩形流程卡片 + 步骤连接线。
- 示例：
```json
{
...
```json
{
  ...
  "title": "技术方案对比",
  "content": [
    "评估维度 | 方案 A (云原生) | 方案 B (传统架构)",
    "部署速度 | 數分鐘快速部署 | 數天手動設置",
    "運維成本 | 自動擴縮容，成本低 | 需專人維護，成本高",
    "可用性 | 99.99% 高可用 | 依賴單點，可用性低"
  ]
}
```

### GeometryGrid（几何网格）
- 用途：科技感的网格化核心要点展示，使用抽象几何色块/卡片分隔，避免排版单一。
- 结构：标题 + 2-4 个网格化几何卡片。
- 示例：
```json
{
  ...
  "title": "系統核心特性",
  "content": [
    "高效處理 :: 採用並行異步執行架構，大幅減少生成時間",
    "極致安全 :: 全程本地或加密通道傳輸，不洩露敏感數據",
    "彈性擴展 :: 模組化插件設計，輕鬆對接第三方服務"
  ]
}
```
