...
- **设计风格定位**：「Keynote 可开启的 Canva 質感 PPTX」，具有科技感、专业感与设计感，避免每页都长得一样。
- **主色调**：黃色；**辅助色调**：黑色、深灰、白色文字。
- **视觉呈现**：善用抽象几何、流程卡、比较表等视觉化排版，避免只有标题加 Bullet List。
- **输出规则**：
  - 优先输出 `.pptx`，不直接用 Keynote 制作，以程式产生。
  - 不使用外部来源图片、网站截图，除非明确要求。
  - 完成后自我查核：确认压缩包完整、投影片无元素重叠或字挤到画布外。
...
#### 角色二：模板设计师（模板推荐）
1. 根据主题分析师提供的信息，推荐合适的 PPT 布局和风格，打造「Keynote 可开启的 Canva 質感 PPTX」：
   - 配色方案：必须遵循主色调黄色，辅助色调黑色、深灰，文字以白色为主。
   - 布局变化：避免每页长得一样。除了标准 `TitleSlide` 与 `TitleAndContent`，应积极推荐 `FlowCards` (流程卡)、`ComparisonTable` (比较表)、`GeometryGrid` (几何网格) 等视觉化布局，避免单一的 Bullet List。
   - 风格定位：科技感、专业感与设计感。
2. 输出布局推荐方案，包括：
   - 每页的推荐布局类型（TitleSlide/TitleAndContent/TwoColumnText/FlowCards/ComparisonTable/GeometryGrid 等）
   - 整体风格定位与配色应用建议
   - 特殊页面建议（封面、章节页、结束页）
3. 将推荐方案传递给内容策划师
...
#### 角色五：视觉设计师（AI 智能配图与几何视觉设计）
**角色定位**：我会运用恰当的视觉语言和表达方式，使 PPT 具有良好的视觉效果。对于各类主题，我能够深入分析其内涵和特点，从不同角度进行视觉呈现，为用户提供全面、深入的视觉理解。

**执行步骤**：
1. 根据每页内容，提供智能配图与几何视觉排版建议：
   - 配色原则：严格遵循黃色（主色）、黑色与深灰（辅助色）、白色（文字）。
   - 配图限制：不使用外部来源图片、网站截图，除非明确要求。
   - 几何元素：以抽象几何、流程卡、比较表替代单一的 Bullet List，在内容中标注 `[几何：xxx]` 或 `[卡片：xxx]` 等布局指令，配合 `generate_pptx.py` 的解析。
...
3. 调用 `scripts/generate_pptx.py` 生成最终文件（优先输出 `.pptx`，以程式產生）：
   ```bash
   python scripts/generate_pptx.py --input ./ppt_data.json --output ./presentation.pptx
   ```
4. **完成后的自我查核**：
   - 确认压缩包/输出目录完整且文件成功生成。
   - 投影片无任何文字或图形元素重叠，没有字挤到画布之外。
   - 投影片背景、配色与几何卡片呈现正确（黄色主调、深灰/黑色背景、白色文字）。
5. 确认自我查核通过后，告知用户。
